...
1. Самое тяжелое к пониманию лежит в `service.py` в методе `upsert_records`. Возможно алгоритмически можно было лучше выполнить, но в моем понимании у нас есть данные, которые нужно проверять перед вставкой и ограничения в виде отсутствие идентификатора
