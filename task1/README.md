...
6. Логгирование на уровне INFO происходит на операциях DDL и DML. Ошибка логируется в `dict_to_pg.py` и изменения не комитятся (если происходит любая psycopg ошибка). Можно усправлять с rollback() каждой транзакцией, но я решил управлять целиком всеми запросами.
